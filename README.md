...
- **成员工作台**：状态驱动四区视图（进行中 / 待办 / 阻塞 / 已完成），拖拽排活、WIP 限制、阻塞必带原因
- **项目看板**：Kanban 视图，按"待办 / 进行中 / 阻塞 / 已完成"分列；支持项目编辑 / 归档 / 删除
...
工作台按 `status` 分四区：**进行中 / 待办 / 阻塞中 / 已完成**。点击「阻塞」按钮会强制弹窗让你写阻塞原因，避免任务卡住却没人知道为什么。
